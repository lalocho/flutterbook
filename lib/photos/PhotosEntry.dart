...
class PhotosEntry extends StatelessWidget with Avatar {
  ...
  PhotosEntry() {
    _nameEditingController.addListener(() {
      photosModel.entityBeingEdited.name = _nameEditingController.text;
    });
    _phoneEditingController.addListener(() {
      photosModel.entityBeingEdited.phone = _phoneEditingController.text;
    });
    _emailEditingController.addListener(() {
      photosModel.entityBeingEdited.email = _emailEditingController.text;
    });
  }
                    ...
                    child: ListView(
                        children: [
                          ListTile(
                              ...
                              Image.memory(
                                ...
                                fit: BoxFit.contain,
                                  ...
                                  : Text("No avatar image for this photo"),
                              trailing: IconButton(
                          ...
                          ListTile(
                            ...
                            title: Text("Birthday"),
                            subtitle: Text(photosModel.chosenDate == null ? "" : photosModel.chosenDate),
                            trailing: IconButton(
                                icon: Icon(Icons.edit),
                                color: Colors.blue,
                                onPressed: () async {
                                  String chosenDate = await utils.selectDate(context,
                                      photosModel, photosModel.entityBeingEdited.birthday);
                                  if (chosenDate != null) {
                                    photosModel.entityBeingEdited.birthday = chosenDate;
                                  }
                                }),
                          )
                        ]
  ...
  void _save(BuildContext context, PhotosModel model) async {
    if (!_formKey.currentState.validate()) {
      return;
    }
    int id = 0;
    if (model.entityBeingEdited.id == null) {
      id = await PhotosDBWorker.db.create(photosModel.entityBeingEdited);
    } else {
      id = await PhotosDBWorker.db.update(photosModel.entityBeingEdited);
    }
    File avatarFile = avatarTempFile();
    if (avatarFile.existsSync()) {
      File f = avatarFile.renameSync(avatarFileName(id));

      // FIXME: force to reload the avartar in the photosList
    }
    ...
    Scaffold.of(context).showSnackBar(
        SnackBar(
          ...
          duration: Duration(seconds: 2), content: Text('photo saved'),
        )
    );
  }
}